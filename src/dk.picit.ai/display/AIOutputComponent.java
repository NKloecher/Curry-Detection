...
public class AIOutputComponent extends JComponent implements MouseListener {
    
	private static final long serialVersionUID = 1685443253803077186L;
	private static final Random RND = new Random();
	private static final int STACKSIZE = 100;
	private static final int STACKWIDTH = 200;
    ...
    AIOutputComponent(Net net, int width, int height, int iterations) {
    	this.net = net;
    ...
    AIOutputComponent(Net net, int width, int height, double xmin, double xmax, double ymin, double ymax, int iterations) {
    ...
    public void paintComponent(Graphics g) {
        ...
        for(Flavor flavor : net.flavors) {
    		...
    		for(int j = 0; j < STACKWIDTH; j++) {
                ...
                if(pselected[i]) g2d.fillOval(x, y, 2, 2);
               
                y = itop  + ih - (int)(1.0*ih*((dyn - ymin)/(ymax - ymin)));
                if(nselected[i]) g2d.fillOval(x, y, 2, 2);
                
                y = itop  + ih - (int)(1.0*ih*((dys - ymin)/(ymax - ymin)));
                if(sselected[i]) g2d.fillOval(x, y, 2, 2);
    		}
...
//        g.drawString("" + pos, x, itop + ih + ibottom - 3);
    ...
    public void updateComponent(Sample sample) {
			...
			if(sampleTarget != null) {
				...
				for(int z = flavor.firstFlavoridx; z < flavor.firstFlavoridx + 64; z++) {
					for(int j = 0; j < posVals[i][curidx].length; j++) {
						if(oVal[z] > -0.009 && oVal[z] <= 0.009) continue;
    	...
    	if (lastUpdate < System.currentTimeMillis() - 200) {
    		...
    		for(int ii = 0; ii < net.flavors.length; ii++) {
				for(int j = 0; j < posVals[ii][curidx].length; j++) {
					posVals[ii][curidx][j] = 0;
					negVals[ii][curidx][j] = 0;
				}
    		}
